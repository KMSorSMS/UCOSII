...
void rate_pid(void *args){
	...
	while(1){
		...
		Motor[0] = cast_to_range((int16_t)(Thro + Pitch - Roll + Yaw ), 1000, 2000);    //M1
		Motor[3] = cast_to_range((int16_t)(Thro - Pitch + Roll + Yaw ), 1000, 2000);    //M4 
		Motor[1] = cast_to_range((int16_t)(Thro + Pitch + Roll - Yaw ), 1000, 2000);    //M2
		print_rate += 1;
		if(print_rate % 100 == 0) {
